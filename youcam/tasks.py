...
from .models import YouCamAnalysis, AnalysisStatus
from .youcam_client import YouCamClient, YouCamAPIError
...
def process_youcam_analysis(self, analysis_id):
    ...
    try:
        ...
        logger.info(f"Starting YouCam analysis for {analysis.analysis_type}: {analysis_id}")
        ...
        logger.error(f"Unexpected error processing YouCam analysis {analysis_id}: {str(exc)}")
