...
from .serializers import (
    YouCamAnalysisCreateSerializer, YouCamAnalysisListSerializer,
    YouCamAnalysisDetailSerializer, YouCamAnalysisRetrySerializer,
    AnalysisHistorySerializer, AnalysisFeedbackSerializer,
    YouCamAnalysisStatsSerializer, BatchAnalysisSerializer
)
from .tasks import process_youcam_analysis, retry_failed_youcam_analyses, generate_analysis_report
...
class YouCamAnalysisViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        analysis = serializer.save(user=request.user if request.user.is_authenticated else None)
    ...
    def statistics(self, request):
        ...
        pending_analyses = YouCamAnalysis.objects.filter(status=AnalysisStatus.PENDING).count()
        processing_analyses = YouCamAnalysis.objects.filter(status=AnalysisStatus.PROCESSING).count()
        completed_analyses = YouCamAnalysis.objects.filter(status=AnalysisStatus.COMPLETED).count()
        failed_analyses = YouCamAnalysis.objects.filter(status=AnalysisStatus.FAILED).count()
