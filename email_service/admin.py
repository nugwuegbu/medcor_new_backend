...
class EmailRequestAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Email Content", {"fields": ("subject", "message", "file_attached")}),
        (
            "Status & Tracking",
            {"fields": ("status", "error_message", "retry_count", "max_retries")},
        ),
        ("Task Information", {"fields": ("celery_task_id",), "classes": ("collapse",)}),
        (
            "Timestamps",
            {
    ...
    def retry_failed_emails(self, request, queryset):
        ...
        self.message_user(
            request, f"Successfully queued {count} failed emails for retry."
        )
    ...
    def mark_as_sent(self, request, queryset):
        ...
        self.message_user(
            request, f"Successfully marked {updated} email requests as sent."
        )
