...
from .serializers import (
    EmailRequestSerializer, EmailRequestDetailSerializer, EmailRequestListSerializer
)
from .tasks import send_email_task, retry_failed_emails
...
class EmailRequestViewSet(viewsets.ModelViewSet):
    ...
    def statistics(self, request):
        ...
        with_attachments = EmailRequest.objects.filter(file_attached__isnull=False).exclude(file_attached='').count()
        ...
        return Response({
            ...
            'success_rate': round((sent_requests / total_requests * 100), 2) if total_requests > 0 else 0
        })
