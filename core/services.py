...
class OpenAIService:
    ...
    async def generate_chat_response(self, message: str, language: str = 'en', context: Dict = None) -> str:
        ...
        try:
            ...
            if context and context.get('history'):
                ...
                for hist in context['history'][-5:]:  # Last 5 messages
                    ...
                    messages.insert(-1, {"role": "assistant", "content": hist['response']})
            ...
            response = await self.client.chat.completions.create(
                ...
                messages=[
                    ...
                    },
                    {
                        "role": "user",
                        "content": [
                            {"type": "text", "text": "Give a brief compliment about their appearance."},
...
class HeyGenService:
    ...
    def __init__(self):
        self.api_key = os.getenv('HEYGEN_API_KEY', 'Mzk0YThhNTk4OWRiNGU4OGFlZDZiYzliYzkwOTBjOGQtMTcyNjczNDQ0Mg==')
    ...
    async def create_avatar_session(self, session_id: str) -> Dict:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(self.streaming_url, headers=headers, json=data) as response:
    ...
    async def generate_avatar_response(self, text: str, session_id: str, language: str = 'en') -> Dict:
...
class FaceRecognitionService:
            ...
            if azure_key:
                try:
                    self.face_client = FaceClient(
                        os.getenv('AZURE_FACE_ENDPOINT', 'https://eastus.api.cognitive.microsoft.com/'),
    ...
    async def register_face(self, image_base64: str, user_id: str, preferred_language: str) -> Dict:
...
class TextToSpeechService:
    ...
    async def convert_text_to_speech(self, text: str, voice_id: str = None, language: str = 'en') -> bytes:
...
class VoiceConversationManager:
    ...
    async def process_voice_input(self, message: str, session_id: str, state: Dict = None) -> Dict:
        ...
        try:
            ...
            response = await self._generate_contextual_response(message, session, voice_command)
    ...
    async def _generate_contextual_response(self, message: str, session: Dict, voice_command: Dict) -> Dict:
