...
from .chat_serializers import (
    ...
    VoiceChatSerializer, TextChatSerializer,
    FaceRecognitionSerializer, FaceRecognizeSerializer,
    FaceRegisterSerializer, AvatarRecordingSerializer,
    AnalysisReportSerializer, ConsentRecordSerializer,
    ConsentRecordRequestSerializer, SpeechToTextSerializer,
    TextToSpeechSerializer, LocationWeatherSerializer
)
from .services import (
    OpenAIService, HeyGenService, FaceRecognitionService,
    TextToSpeechService, SpeechToTextService,
    WeatherService, VoiceConversationManager
)
...
class VoiceChatView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            is_first_response = not ChatMessage.objects.filter(
                session=session,
                message__isnull=False
            ).exclude(message='').exists()
            
            # Analyze user image if provided
            compliment = ""
            if data.get('user_image'):
                compliment = async_to_sync(openai_service.analyze_user_image)(data['user_image'])
        ...
        try:
            ...
            if result.get('success'):
                ...
                FaceRecognition.objects.update_or_create(
                    user=user,
                    defaults={
                        'face_encoding': data['image_base64'][:100],  # Store partial for demo
...
class FaceAnalysisReportView(APIView):
    ...
    def post(self, request):
        ...
        required_fields = ['patientName', 'patientEmail', 'patientPhone', 'patientJob', 'analysisResult']
        ...
        try:
            ...
            report = AnalysisReport.objects.create(
                user=request.user if request.user.is_authenticated else None,
                hospital_id=request.user.hospital_id if request.user.is_authenticated else 1,
                report_type='face',
