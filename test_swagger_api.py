...
def test_api_documentation():
    ...
    try:
        ...
        (
            print(f"   - Base Path: {schema.get('servers', [{}])[0].get('url', '/')}")
        ...
        if tags:
            ...
            for tag in tags:
                print(
                    f"   - {tag.get('name')}: {tag.get('description', 'No description')}"
                )
        ...
        for i, (path, operations) in enumerate(list(paths.items())[:10]):
            ...
            if i == 0:
                for method, details in operations.items():
                    if method in ["get", "post", "put", "patch", "delete"]:
                        print(
                            f"      - {method.upper()}: {details.get('summary', 'No summary')}"
                        )
        ...
        auth_endpoints = [
            p
            for p in paths.keys()
            if "auth" in p.lower() or "login" in p.lower() or "register" in p.lower()
        ]
        ...
        hospital_endpoints = [
            p for p in paths.keys() if "hospital" in p.lower() or "tenant" in p.lower()
        ]
        ...
        if security_schemes:
            ...
            for name, scheme in security_schemes.items():
                print(
                    f"   - {name}: {scheme.get('type')} ({scheme.get('scheme', 'N/A')})"
                )
    ...
    try:
        ...
        doc_urls = [
            ...
            if any(
                term in str(p.pattern)
                for term in ["swagger", "redoc", "schema", "docs"]
            )
        ]
        if doc_urls:
            print(f"\n📚 Documentation URLs:")
