...
class SubscriptionPlanSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'currency', 'billing_cycle', 'trial_days', 'max_users',
            'max_doctors', 'max_patients', 'max_appointments_per_month',
            'storage_gb', 'has_telemedicine', 'has_analytics',
            'has_api_access', 'has_custom_branding', 'has_priority_support',
            'has_data_export', 'has_multi_location', 'has_advanced_reporting',
            'features', 'is_active', 'is_featured', 'display_order',
            'created_at', 'updated_at', 'metadata', 'monthly_price'
        ]
...
class SubscriptionPlanListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = SubscriptionPlan
        fields = [
            'id', 'name', 'slug', 'plan_type', 'price', 'currency',
            'billing_cycle', 'monthly_price', 'is_featured', 'features'
        ]
...
class SubscriptionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Subscription
        fields = [
            'id', 'hospital', 'hospital_name', 'plan', 'plan_name',
            'plan_details', 'status', 'start_date', 'end_date',
            'trial_end_date', 'cancelled_at', 'current_period_start',
            'current_period_end', 'next_billing_date', 'payment_method',
            'last_payment_date', 'last_payment_amount', 'current_users',
            'current_doctors', 'current_patients',
            'current_appointments_this_month', 'current_storage_gb',
            'auto_renew', 'created_at', 'updated_at', 'metadata',
            'days_until_renewal', 'is_in_trial', 'usage_percentage'
        ]
        read_only_fields = [
            'id', 'created_at', 'updated_at', 'plan_name', 'hospital', 'hospital_name',
            'days_until_renewal', 'is_in_trial', 'usage_percentage'
        ]
    ...
    def get_usage_percentage(self, obj):
        ...
        if obj.plan:
            ...
            if obj.plan.storage_gb > 0:
                usage['storage'] = (float(obj.current_storage_gb) / obj.plan.storage_gb) * 100
        return usage
    ...
    def validate(self, data):
        ...
        if 'current_period_end' in data and 'current_period_start' in data:
            if data['current_period_end'] < data['current_period_start']:
                raise serializers.ValidationError("Current period end must be after start")
...
class CreateSubscriptionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Subscription
        fields = [
            'hospital', 'plan', 'status', 'start_date', 'end_date',
            'trial_end_date', 'current_period_start', 'current_period_end',
            'next_billing_date', 'payment_method', 'auto_renew', 'metadata'
        ]
    ...
    def create(self, validated_data):
        ...
        if plan.trial_days > 0 and 'trial_end_date' not in validated_data:
            validated_data['trial_end_date'] = (
                validated_data['start_date'] + timezone.timedelta(days=plan.trial_days)
            )
        ...
        if 'next_billing_date' not in validated_data and validated_data.get('auto_renew'):
