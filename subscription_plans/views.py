...
class SubscriptionPlanViewSet(viewsets.ModelViewSet):
    ...
    def compare(self, request):
        ...
        if not plan_ids:
            return Response(
                {"error": "plan_ids are required"}, status=status.HTTP_400_BAD_REQUEST
            )
...
class SubscriptionViewSet(viewsets.ModelViewSet):
    ...
    def expiring_soon(self, request):
        ...
        queryset = self.get_queryset().filter(
            end_date__lte=thirty_days_from_now, status__in=["ACTIVE", "TRIAL"]
        )
    ...
    def usage(self, request, pk=None):
        ...
        usage_data = {
            ...
            "users": {
                ...
                "percentage": (
                    (subscription.current_users / subscription.plan.max_users * 100)
            ...
            "doctors": {
                ...
                "percentage": (
                    (subscription.current_doctors / subscription.plan.max_doctors * 100)
                    if subscription.plan.max_doctors > 0
                    else 0
                ),
            },
            "patients": {
                ...
                "percentage": (
                    (
                        subscription.current_patients
                        / subscription.plan.max_patients
                        * 100
                    )
                    if subscription.plan.max_patients > 0
                    else 0
                ),
            },
            "appointments": {
                ...
                "percentage": (
                    (
                        subscription.current_appointments_this_month
                        / subscription.plan.max_appointments_per_month
                        * 100
                    )
                    if subscription.plan.max_appointments_per_month > 0
                    else 0
                ),
            },
            "storage": {
                ...
                "percentage": (
                    (
                        float(subscription.current_storage_gb)
                        / subscription.plan.storage_gb
                        * 100
                    )
                    if subscription.plan.storage_gb > 0
                    else 0
                ),
            },
            "is_over_limit": False,
