...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Hair Analysis Page Camera Routes
  app.post("/api/hair-camera/init", async (req, res) => {
    try {
      const { sessionId } = req.body;
      ...
      const pageState = pageCameraService.initPageCamera(sessionId, PAGE_IDS.HAIR);
      console.log(`📄 BACKEND: Hair camera initialized for session ${sessionId}`);
      
      res.json({ 
        success: true, 
        pageState,
        message: "Hair camera initialized"
      });
    } catch (error) {
      console.error("Hair camera init error:", error);
      res.status(500).json({ message: "Failed to initialize hair camera" });
    }
  });

  app.get("/api/hair-camera/status/:sessionId", async (req, res) => {
    try {
      const { sessionId } = req.params;
      
      const pageState = pageCameraService.getPageCamera(sessionId, PAGE_IDS.HAIR);
      const isAvailable = pageCameraService.isPageCameraAvailable(sessionId, PAGE_IDS.HAIR);
      
      res.json({ 
        success: true,
        pageState,
        isAvailable,
        message: pageState ? "Hair camera found" : "Hair camera not initialized"
      });
    } catch (error) {
      console.error("Hair camera status error:", error);
      res.status(500).json({ message: "Failed to get hair camera status" });
    }
  });

  app.post("/api/hair-camera/disable", async (req, res) => {
    try {
      const { sessionId } = req.body;
      ...
      const success = pageCameraService.disablePageCamera(sessionId, PAGE_IDS.HAIR);
      console.log(`📄 BACKEND: Hair camera disabled for session ${sessionId}`);
      
      res.json({ 
        success,
        message: success ? "Hair camera disabled" : "Hair camera not found"
      });
    } catch (error) {
      console.error("Hair camera disable error:", error);
      res.status(500).json({ message: "Failed to disable hair camera" });
    }
  });

  app.post("/api/hair-camera/enable", async (req, res) => {
    try {
      const { sessionId } = req.body;
      ...
      const success = pageCameraService.enablePageCamera(sessionId, PAGE_IDS.HAIR);
      console.log(`📄 BACKEND: Hair camera enabled for session ${sessionId}`);
      
      res.json({ 
        success,
        message: success ? "Hair camera enabled" : "Hair camera not found"
      });
    } catch (error) {
      console.error("Hair camera enable error:", error);
      res.status(500).json({ message: "Failed to enable hair camera" });
    }
  });
