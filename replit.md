...
- July 29, 2025. CIRCULAR FLOATING AVATAR BUG FIXED - Removed unnecessary "Simple Avatar Circle for doctor list" that was causing a purple circular avatar with "AI" text to appear when the menu was open. This floating avatar was an extra component that wasn't needed since the main HeyGen avatar handles all avatar displays properly.
- July 31, 2025. FLOATING PURPLE AI AVATAR FIX REAPPLIED - The floating purple circular avatar bug reappeared. After extensive search through codebase, couldn't locate the specific element. Implemented comprehensive MutationObserver-based fix that detects and removes any floating avatar elements containing "AI" text when the circular menu is shown (showChatInterface is true). Fix includes multiple selectors to catch various implementations and logs removals for debugging.
