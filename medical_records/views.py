...
from .serializers import (
    ...
    CreateMedicalRecordSerializer, MedicalDocumentSerializer,
...
class MedicalRecordViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.role == 'PATIENT':
            ...
            queryset = queryset.filter(
                Q(created_by=user) | Q(patient__doctor=user)
            )
        elif user.role == 'NURSE':
            # Nurses can see non-confidential records
            queryset = queryset.filter(
                Q(created_by=user) | Q(is_confidential=False)
            )
        ...
        return queryset.select_related('patient', 'created_by').prefetch_related('documents')
    ...
    def perform_create(self, serializer):
        ...
        hospital_name = None
        if hasattr(self.request.user, 'hospital_name') and self.request.user.hospital_name:
    ...
    def patient_history(self, request):
        ...
        history_data = {
            ...
            'records_by_type': dict(
                records.values('record_type').annotate(count=Count('id')).values_list('record_type', 'count')
            ),
            'recent_diagnoses': list(
                records.filter(record_type='DIAGNOSIS').order_by('-created_at')[:5].values(
                    'title', 'diagnosis', 'created_at'
                )
            ),
            'allergies': list(
            ...
            'recent_lab_results': list(
                records.filter(record_type='LAB_RESULT').order_by('-created_at')[:5].values(
                    'title', 'lab_results', 'created_at'
                )
            ),
