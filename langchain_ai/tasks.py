...
def process_ai_query_task(self, query_data: Dict[str, Any], user_id: str = None):
    ...
    This is used for complex queries that require multiple tool calls or
    extensive processing
    """
    try:
        # Update task status
        self.update_state(
            state="PROGRESS", meta={"status": "Processing query...", "progress": 10}
        )
...
@shared_task(bind=True, max_retries=3)
def process_medical_analysis_task(
    self, medical_data: Dict[str, Any], user_id: str = None
):
    """
    ...
    try:
        ...
        result = asyncio.run(
            _process_medical_analysis_async(data, analysis_type, patient_id)
        )
        ...
        task_id = self.request.id
        cache.set(
            f"medical_analysis_result_{task_id}", result, timeout=7200
        )  # 2 hours cache
...
def process_database_query_task(self, query_data: Dict[str, Any], user_id: str = None):
    ...
    try:
        ...
        task_id = self.request.id
        cache.set(
            f"db_query_result_{task_id}", result, timeout=1800
        )  # 30 minutes cache
...
def process_code_analysis_task(self, code_data: Dict[str, Any], user_id: str = None):
    ...
    try:
        # Update task status
        self.update_state(
            state="PROGRESS", meta={"status": "Analyzing code...", "progress": 25}
        )
        ...
        result = asyncio.run(
            _process_code_analysis_async(code, language, analysis_type)
        )
        ...
        task_id = self.request.id
        cache.set(
            f"code_analysis_result_{task_id}", result, timeout=1800
        )  # 30 minutes cache
...
async def _process_medical_analysis_async(
    data: str, analysis_type: str, patient_id: str = None
) -> Dict[str, Any]:
...
async def _process_code_analysis_async(
    code: str, language: str, analysis_type: str
) -> Dict[str, Any]:
...
def get_task_status(task_id: str) -> Dict[str, Any]:
    ...
    if task_result.state == "PENDING":
        ...
        return {"status": task_result.state, "message": "Unknown task state"}
