...
def process_query(request):
    ...
    try:
        ...
        if not use_celery and len(query) < 500:
            result = asyncio.run(_process_simple_query_async(query, query_type, context))
...
def process_database_query(request):
    """Process database queries using async for simple queries, Celery for complex ones"""
    query = request.data.get('query', '')
    ...
    try:
        # For simple queries, use async processing
        if len(query) < 200 and not any(keyword in query.upper() for keyword in ['JOIN', 'GROUP BY', 'ORDER BY', 'HAVING']):
...
async def _process_simple_query_async(query: str, query_type: str, context: Dict[str, Any]) -> Dict[str, Any]:
