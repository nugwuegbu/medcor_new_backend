...
class QueryRequestSerializer(serializers.Serializer):
    ...
    query = serializers.CharField(
        max_length=2000,
        help_text="The user's query or prompt"
    )
    ...
    context = serializers.DictField(
        required=False,
        allow_empty=True,
        help_text="Additional context for the query"
    )
...
class MedicalQueryRequestSerializer(serializers.Serializer):
    ...
    medical_data = serializers.CharField(
        max_length=5000,
        help_text="Medical data or query to analyze"
    )
    ...
    patient_id = serializers.CharField(
        required=False,
        allow_blank=True,
        help_text="Patient ID (if applicable)"
    )
...
class DatabaseQueryRequestSerializer(serializers.Serializer):
    ...
    query = serializers.CharField(
        max_length=2000,
        help_text="SQL query or database request"
    )
    database = serializers.CharField(
        max_length=100,
        default="default",
        help_text="Database name to query"
    )
...
class QueryResponseSerializer(serializers.Serializer):
    ...
    query_type = serializers.CharField(
        help_text="Type of query that was processed"
    )
    tools_used = serializers.ListField(
        child=serializers.CharField(),
        help_text="List of tools that were used"
    )
    processing_time = serializers.FloatField(
        required=False,
        help_text="Time taken to process the query in seconds"
    )
    ...
    error = serializers.CharField(
        required=False,
        allow_blank=True,
        help_text="Error message if the query failed"
    )
...
class HealthCheckSerializer(serializers.Serializer):
    ...
    status = serializers.CharField(
        help_text="Overall status of the service"
    )
    mcp_available = serializers.BooleanField(
        help_text="Whether MCP server is available"
    )
