...
class AsyncMCPClient:
    ...
    def __init__(self, mcp_server_url: str = None):
        self.mcp_server_url = mcp_server_url or getattr(
            settings, "MCP_SERVER_URL", "http://localhost:8001"
        )
    ...
    async def call_tool(
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                return MCPResponse(
                    success=True, data=data.get("result"), metadata=data.get("metadata")
                )
            else:
                return MCPResponse(
                    success=False,
    ...
    async def health_check(self) -> MCPResponse:
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                return MCPResponse(
                    success=True, data=data, metadata={"status": "healthy"}
                )
            else:
                return MCPResponse(
                    success=False,
