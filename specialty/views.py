...
class SpecialtyViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = queryset.annotate(
            doctor_count=Count('specialists', distinct=True)
        )
    ...
    def popular(self, request):
        ...
        specialties = self.get_queryset().annotate(
            total_doctors=Count('specialists')
    ...
    def doctors(self, request, pk=None):
        ...
        doctor_specialties = DoctorSpecialty.objects.filter(
            specialty=specialty
        ).select_related('doctor', 'specialty').order_by('-is_primary', 'doctor__last_name')
    ...
    def update_statistics(self, request, pk=None):
        ...
        stats, created = SpecialtyStatistics.objects.get_or_create(
            specialty=specialty
        )
...
class DoctorSpecialtyViewSet(viewsets.ModelViewSet):
    ...
    search_fields = ['doctor__first_name', 'doctor__last_name', 
                    'specialty__name', 'certification_number']
...
class SpecialtyStatisticsViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    ordering_fields = ['total_doctors', 'total_appointments', 
                      'average_rating', 'average_wait_days']
    ...
    def top_rated(self, request):
        ...
        top_specialties = self.get_queryset().filter(
    ...
    def high_demand(self, request):
        ...
        high_demand = self.get_queryset().filter(
            appointment_requests_last_month__gt=50
