...
class Specialty(models.Model):
    ...
    code = models.CharField(
        max_length=50, choices=SPECIALIZATION_CHOICES, unique=True, db_index=True
    )
...
class DoctorSpecialty(models.Model):
    ...
    specialty = models.ForeignKey(
        Specialty, on_delete=models.CASCADE, related_name="specialists"
    )

    # Additional fields for the relationship
    is_primary = models.BooleanField(
        default=False, help_text="Is this the doctor's primary specialization?"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.is_primary:
            DoctorSpecialty.objects.filter(doctor=self.doctor, is_primary=True).exclude(
                pk=self.pk
...
class SpecialtyStatistics(models.Model):
    ...
    specialty = models.OneToOneField(
        Specialty, on_delete=models.CASCADE, related_name="statistics"
    )
    ...
    average_wait_days = models.DecimalField(
