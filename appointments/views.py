...
class DoctorAvailabilitySlotViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if available_only:
            queryset = queryset.filter(
                status="AVAILABLE", start_time__gte=timezone.now()
            ).exclude(current_appointments__gte=F("max_appointments"))
    ...
    def available(self, request):
        ...
        if date_str:
            try:
                ...
                start_datetime = (
                    timezone.make_aware(slot_date)
                    if timezone.is_naive(slot_date)
                    else slot_date
                )
                end_datetime = start_datetime.replace(hour=23, minute=59, second=59)
                queryset = queryset.filter(
                    start_time__gte=start_datetime, start_time__lte=end_datetime
                )
            except ValueError:
                return Response(
            ...
            while current_date <= end:
                ...
                for slot_config in day_slots:
                    ...
                    existing = DoctorAvailabilitySlot.objects.filter(
                        doctor=doctor, start_time=start_datetime, end_time=end_datetime
                    ).first()
    ...
    def unblock(self, request, pk=None):
        ...
        if slot.status != "BLOCKED":
            return Response(
                {"error": "Slot is not blocked"}, status=status.HTTP_400_BAD_REQUEST
            )
...
class AppointmentViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        upcoming_only = (
            self.request.query_params.get("upcoming", "false").lower() == "true"
        )
        if upcoming_only:
    ...
    def dashboard_stats(self, request):
        ...
        if request.user.role == "DOCTOR":
            stats["total_patients"] = (
                queryset.filter(doctor=request.user)
                .values("patient")
                .distinct()
                .count()
            )
