...
class Appointment(models.Model):
    ...
    def clean(self):
        ...
        # Validate doctor and patient belong to same hospital (if hospital is specified)
        if self.doctor_id and self.patient_id and self.hospital_id:
...
class DoctorAvailabilitySlot(models.Model):
    ...
    current_appointments = models.IntegerField(
        default=0, help_text="Current number of appointments booked in this slot"
    )

    # Status
    status = models.CharField(
        max_length=20, choices=SLOT_STATUS_CHOICES, default="AVAILABLE"
    )
    ...
    advance_booking_days = models.IntegerField(
        default=30, help_text="How many days in advance this slot can be booked"
    )
    minimum_notice_hours = models.IntegerField(
        default=24, help_text="Minimum hours notice required for booking"
    )
    ...
    created_by = models.ForeignKey(
        "core.User", on_delete=models.SET_NULL, null=True, related_name="created_slots"
    )
    ...
    def clean(self):
        ...
        # Validate doctor belongs to hospital (if hospital is specified)
    ...
    def is_available(self):
        ...
        return (
            ...
            and self.current_appointments < self.max_appointments
